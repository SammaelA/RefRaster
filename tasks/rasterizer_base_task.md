# Задание 1. Базовый растеризатор
...
* **Использовать OpenGL/Vulkan/DirectX для растеризации запрещено.** Вся
  ...
  (как в шаблоне).
* Кадровый буфер хранится в `float` (не в 8-битных цветах): это понадобится
  в следующих заданиях. Перевод в 8 бит и запись в файл/окно — отдельный шаг
  в самом конце кадра.
* Программа принимает путь к модели и размер картинки параметрами командной
  строки.

## Часть 1 — offline, сохранение в картинку

Эта часть задания состоит из нескольких этапов, за каждый из 
которых начисляется указанное число баллов. Для каждого этапа написан
ожидаемый результат (как понять, что этап выполнен), но это сделано 
скорее для вашего удобства - при сдаче задания можно показать только
общий результат.
...
## Часть 2 — online, вывод в окно

## Ожидаемый результат

1. Изображение выводится в окно заданного размера в реальном времени (>30 FPS).
Если частота кадров низкая - уменьшите разрешение. Если при разрешении 640х480 
у вас все еще нет 30 FPS и ваш компьютер выпущен после 2010 года - проблема в реализации,
...
### Пункт 6. Вывод в окно и управление камерой (4 балла)
...
### Пункт 7. Naive upscaling (2 балла)
...
| 6 | Окно, реальное время, управление камерой | 4 |
| 7 | Рендер в пониженном разрешении и апскейл | 2 |
| | **Итого** | **20** |
...
Использование готовых графических API для растеризации или готовых
математических библиотек для матриц обнуляет баллы за соответствующие пункты.
